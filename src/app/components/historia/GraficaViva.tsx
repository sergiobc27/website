...
import {
  ...
  ReferenceDot, ResponsiveContainer, Scatter, Tooltip, XAxis, YAxis,
} from 'recharts';
...
/**
 * La visualización única del scrollytelling: recibe la escena activa (1-8) y
 * rinde la variante correspondiente. Decorativa para lectores de pantalla
 * (aria-hidden): la narrativa textual de cada escena ES el contenido accesible.
 */
        ...
        {escena === 2 && (
          <ChartCon titulo={`Tormenta real del ${tormenta.fecha} — pulsos de 10 minutos`}>
            <BarChart data={tormenta.puntos}>
              ...
              <Bar dataKey="mm" fill="#2563eb" isAnimationActive animationDuration={550} />
            </BarChart>
          </ChartCon>
        )}
          ...
          <div className="flex h-full flex-col gap-2">
            ...
            <ChartCon titulo="Lo que vería la práctica común: un solo dato diario" altura="flex-[2]">
              <BarChart data={[{ nombre: tormenta.fecha, mm: tormenta.totalMm }]} layout="vertical">
                ...
                <Bar dataKey="mm" fill="var(--accent)" radius={[0, 4, 4, 0]} isAnimationActive animationDuration={550} />
              </BarChart>
            </ChartCon>
          </div>
        )}
        ...
        {escena === 4 && (
          <ChartCon titulo={`La peor tormenta de 24 h de cada año (${data.maximosAnuales.length} años)`}>
            <ComposedChart data={data.maximosAnuales}>
              ...
              <Scatter dataKey="mm" fill="#A3161A" isAnimationActive animationDuration={550} />
              <ReferenceDot
        ...
        {escena === 5 && (
          <ChartCon titulo="Del azar a la probabilidad: ajuste de Gumbel">
            <ComposedChart data={cuantiles}>
              ...
              <Line type="monotone" dataKey="mm" name="Gumbel ajustado" stroke="#C9A227" strokeWidth={2} dot={{ r: 3 }} isAnimationActive animationDuration={550} />
              <Scatter data={data.empiricos} dataKey="mm" name="Observado" fill="#A3161A" isAnimationActive animationDuration={550} />
        ...
        {escena === 6 && (
          <ChartCon titulo={`Curvas IDF reales de ${data.estacion.nombre}`}>
            <ComposedChart>
              ...
              <Area data={curva25.puntos} dataKey="banda" name={`IC 90% (Tr ${curva25.tr})`} stroke="none" fill="#C9A227" fillOpacity={0.15} connectNulls isAnimationActive animationDuration={550} />
              {data.curvas.map((c, i) => (
                <Line
                  key={c.tr}
                  data={c.puntos}
                  ...
                  strokeWidth={c.tr === curva25.tr ? 2.5 : 1.5}
                  dot={false}
                  isAnimationActive
