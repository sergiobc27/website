...
/** Top 5 departamentos por lámina mensual de lluvia (mm/mes = region.monthlyDepth).
 *  Ojo: rankear por la media por lectura (mean) invertía el orden real
 *  (p. ej. Valle por encima de Chocó); la lámina mensual es la magnitud correcta. */
export function CeldaTop({ onClick, indice, className }: { onClick: () => void; indice?: number; className?: string }) {
  ...
  useEffect(() => {
    apiJson<AnalyticsByRegionResponse>(
      ...
      .then((r) => {
        const top = (r.regions || [])
          .filter((x) => x.monthlyDepth != null)
          .sort((a, b) => (b.monthlyDepth as number) - (a.monthlyDepth as number))
          .slice(0, 5)
          .map((x) => ({ nombre: x.department, valor: x.monthlyDepth as number }));
        setFilas(top);
      })
      .catch(() => setError(true))
  ...
  return (
    <Celda
      ...
      ariaLabel={`Ver analítica: los departamentos con mayor lámina mensual de lluvia son ${filas.map((f) => f.nombre).join(', ') || 'cargando'}`}
      onClick={onClick}
      cargando={cargando}
      ...
      <div className="flex flex-1 flex-col justify-center gap-1.5" aria-hidden="true">
        {filas.map((f) => (
          <div key={f.nombre} className="flex items-center gap-2 text-xs">
            ...
            <span className="tabular-nums font-semibold text-card-foreground">{fmt(f.valor, 0)}</span>
          </div>
        ))}
        <p className="mt-1 text-[10px] text-muted-foreground">mm/mes promedio histórico</p>
      </div>
    </Celda>
  );
}
